...
describe('Mobile shelf location', () => {
  it('names the book the route is for', async () => {
    await renderAt(`?book=${book.id}`)
  ...
  it('shows the shelf and floor', async () => {
    await renderAt(`?book=${book.id}`)
   ...
   */
  it('writes out every turn, not only the map', async () => {
    await renderAt(`?book=${book.id}`)
  ...
  it('draws the route to this book’s own aisle', async () => {
    await renderAt(`?book=${book.id}`)
    expect(
   ...
   */
  it('says whether a copy is actually on the shelf', async () => {
    await renderAt(`?book=${book.id}`)
...
describe('Mobile shelf location — codes that lead nowhere', () => {
  ...
  it('distinguishes an unknown book from a missing one', async () => {
    await renderAt('?book=khong-co-cuon-nay')
